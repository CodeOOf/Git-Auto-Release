...
- **Major bump**: When `alpha` branch merges to `main` → `vX.Y.Z` becomes `vX+1.0.0-alpha` (automatic tag created)
  ...
  - During alpha phase (X.0.0-alpha), feature/bugfix merges increment build: `X.0.0-alpha.N`
  - `beta` → `main` merge adds `-beta` suffix to already-bumped version
  - During beta phase (X.0.0-beta), feature/bugfix merges increment build: `X.0.0-beta.N`
- **Minor bump**: When `feature/*` branch merges to `main` → `vX.Y.Z` becomes `vX.Y+1.0-beta` (automatic tag)
  - Exception: During major alpha/beta (X.0.0-alpha or X.0.0-beta), increments build instead: `X.0.0-{alpha|beta}.N`
...
- **Automatically updated by CI/CD** after successful merges:
  ...
  - During alpha phase (X.0.0-alpha), `feature/*` or `bugfix/*` → `main`: VERSION becomes `X.0.0-alpha.N`
  - `beta` → `main`: VERSION becomes `X.0.0-beta`
  - During beta phase (X.0.0-beta), `feature/*` or `bugfix/*` → `main`: VERSION becomes `X.0.0-beta.N`
  - Outside major release phases, `feature/*` → `main`: VERSION becomes `X.Y+1.0-beta`
  - Outside major release phases, `bugfix/*` → `main`: VERSION becomes `X.Y.Z+1-beta`
...
| Branch Pattern | Build Version | Tag on Merge | Example (if VERSION=1.0.0) | Notes |
|---------------|---------------|--------------|-------|-------|
| `alpha` (push/PR) | `vX.Y.Z+SHA` | - | `v1.0.0+nd8290q2` | Build version with SHA |
| `alpha` (merged to main) | - | `vX+1.0.0-alpha` | `v2.0.0-alpha` | VERSION becomes 2.0.0-alpha, beta branch created |
| `beta` (push/PR) | `vX.Y.Z-alpha+SHA` | - | `v2.0.0-alpha+8d92k127` | Build version with SHA |
| `beta` (merged to main) | - | `vX.Y.Z-beta` | `v2.0.0-beta` | VERSION becomes 2.0.0-beta |
| `feature/*` (push/PR normal) | `vX.Y.Z+SHA` | - | `v1.0.0+21AF26D3` | Always VERSION+SHA |
| `feature/*` (PR major alpha/beta) | `vX.0.0-{alpha\|beta}+SHA` | - | `v2.0.0-beta+abc123` | Shows current VERSION+SHA during major release |
| `feature/*` (merged to main) | - | `vX.Y+1.0-beta` or `vX.0.0-{alpha\|beta}.N` | `v1.1.0-beta` or `v2.0.0-beta.1` | Normal dev: MINOR bump. Major alpha/beta: .N increment |
| `bugfix/*` (push) | `vX.Y.Z+SHA` | - | `v1.0.0+21AF26D3` | Build version with SHA |
| `bugfix/*` (PR normal) | `vX.Y.Z-bugfix.N` | - | `v1.0.0-bugfix.1` | Preview (no bump yet) |
| `bugfix/*` (PR major alpha/beta) | `vX.0.0-{alpha\|beta}+SHA` | - | `v2.0.0-beta+abc123` | Shows current VERSION+SHA during major release |
| `bugfix/*` (merged to main) | - | `vX.Y.Z+1-beta` or `vX.0.0-{alpha\|beta}.N` | `v1.0.1-beta` or `v2.0.0-beta.1` | Normal dev: PATCH bump. Major alpha/beta: .N increment |
| `hotfix` (push/PR) | `vX.Y.Z-hotfix.N` | - | `v1.0.0-hotfix.1` | Increment counter for hotfix builds |
...
- **Major release protection**: During major alpha (X.0.0-alpha) or major beta (X.0.0-beta), feature/bugfix merges increment build metadata (.N) instead of version numbers
